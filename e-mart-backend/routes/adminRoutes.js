...
router.post('/users/request-seller-role', ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    let existingRequest = await SellerRequest.findOne({ user: req.user.id, status: 'pending' });
    if (existingRequest) {
...
router.get('/admin/seller-requests', ensureAuthenticated, ensureRole('admin'), async (req, res) => {
  try {
    const requests = await SellerRequest.find({ status: 'pending' }).populate('user', 'displayName email'); // Populate user info
    res.json(requests);
  } catch (error) {
    console.error(error.message);
    res.status(500).send('Server Error');
...
router.post('/users/request-admin-role', ensureAuthenticated, async (req, res) => {
  ...
  try {
    let existingRequest = await AdminRequest.findOne({ user: req.user.id, status: 'pending' });
    if (existingRequest) {
