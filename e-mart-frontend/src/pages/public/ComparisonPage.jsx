...
const ComparisonPage = () => {
  const { comparisonList, removeFromCompare, clearCompareList } =
    useComparison();
  ...
  const comparableAttributes = allAttributes
    .filter(
      (attr) =>
        ![
          '_id',
          ...
          'imageUrl',
          'name',
        ].includes(attr) && typeof products[0][attr] !== 'object'
    )
    .sort();
            ...
            {comparableAttributes.map((attr) => (
              <TableRow key={attr}>
                ...
                {products.map((product) => (
                  <TableCell key={product._id} align='center'>
                    {attr === 'price'
                      ? `$${product[attr].toFixed(2)}`
                      : Array.isArray(product[attr])
                        ? product[attr].join(', ')
                        : product[attr]}
                  </TableCell>
                ))}
