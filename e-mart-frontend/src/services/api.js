...
import { app } from '../firebase'; // Assuming your firebase init file is here
...
const apiRequest = async (url, options = {}) => {
  ...
  if (user) {
    try {
      ...
      // You might want to handle this error, e.g., by redirecting to login
    }
  }
...
// Logout is now a client-side only operation, but we can keep a no-op here
// or remove it entirely if we refactor the context.
export const logout = () => Promise.resolve({ msg: 'Client-side operation.' });

// PRODUCTS
export const fetchProducts = (filters) => {
  const params = new URLSearchParams();
  // Add all possible filters to the params
...
// SELLER
export const fetchSellerProducts = (status) => {
  const params = new URLSearchParams();
  if (status && status !== 'all') {
    params.append('status', status);
  }
  return apiRequest(`/api/products/seller/my-products?${params.toString()}`);
};
